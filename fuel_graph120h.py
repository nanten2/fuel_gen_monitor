...
day_list = ret_filename(ret_path()[-1],num = 6)#最新ディレクトリの中に入っている中で新しいものから6つのファイル名からなるリスト
number_of_days = len(day_list)#最新ディレクトリから取得できたファイル名の数
data = []
time_list = []
t_all = []

if number_of_days < 6:#一番新しいディレクトリにファイルが１つしかない、または全くなかった場合  
    day_list2 = ret_filename(ret_path()[-2],num = 6-number_of_days)
    for i in day_list2:
...
tmin_timestamp = tmax_timestamp-432000#最新のデータ取得時間より120時間前のUNIXtime
num = 0

#最新のデータ取得時間から120時間前までの間に何回データを取得したかを数える 
for t in t_all:
    ...
    if tmin_timestamp <= t_timestamp:
        num = num+1
offset = len(data) - num
data_plot = data[offset:]#120時間より新しいデータの切り出し   
timelist_plot = time_list[offset:]#120時間より新しい時間データの切り出し  
data19 = data_plot
for i in range(19):
...
if not os.path.exists('./static/img'):
    os.mkdir('static/img')
if not os.path.exists('./static/img/120h_img'):
    os.mkdir('./static/img/120h_img')
