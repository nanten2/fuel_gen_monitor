...
day_list = ret_filename(ret_path()[-1],num = 15)#最新ディレクトリの中に入っている中で新しいものから15つのファイル名からなるリスト
number_of_days = len(day_list)#最新ディレクトリから取得できたファイル名の数
data = []
time_list = []
t_all = []

if number_of_days < 15:#一番新しいディレクトリにファイルが14個以下だった場合  
    day_list2 = ret_filename(ret_path()[-2],num = 15-number_of_days)
    for i in day_list2:
...
tmin_timestamp = tmax_timestamp-1209600#最新のデータ取得時間より14日前のUNIXtime
num = 0

#最新のデータ取得時間から14日前までの間に何回データを取得したかを数える 
for t in t_all:
    ...
    if tmin_timestamp <= t_timestamp:
        num = num+1
offset = len(data) - num
data_plot = data[offset:]#14日前より新しいデータの切り出し   
timelist_plot = time_list[offset:]#14日前より新しい時間データの切り出し  
data19 = data_plot
for i in range(19):
    exec("data{},data19 = np.hsplit(data19,[1])".format(i))#各パラメーターごとにデータを分割

if not os.path.exists('./static'):#画像の保存場所作成                                                        
    os.mkdir('static')
if not os.path.exists('./static/img'):
    os.mkdir('static/img')
if not os.path.exists('./static/img/14day_img'):
    os.mkdir('./static/img/14day_img')
...
def plot_fig1(title,Ylabel,data_1):
    ...
    plt.savefig("/home/amigos/fuel_generator_monitor/static/img/14day_img/{}".format(title))
    #plt.show()                                                                                                                                               
...
plot_fig2("highlevelalarm","Voltage[V]",data12,data15,"65063","65064")
plot_fig2("lowlevelalarm","Voltage[V]",data13,data16,"65063","65064")
plot_fig2("solenoid","Voltage[V]",data14,data17,"65063","65064")
